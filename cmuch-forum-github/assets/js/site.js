...
function personInline(ids,label){
  ...
  var h=vis.map(function(p){return '<button class="pbtn" data-person="'+esc(p.id)+'"><b>'+esc(p.name)+'</b> '+esc(p.title)+(p.status!=="confirmed"&&adminView()?'<em class="badge-inv">邀請中</em>':'')+'<span>'+esc(p.org)+'</span></button>'}).join("");
...
function speakersHTML(){
  ...
  var cards=list.map(function(p){
    ...
    '<div><h3>'+esc(p.name)+'<small>'+esc(p.title)+'</small>'+(p.status!=="confirmed"&&adminView()?'<em class="badge-inv">邀請中</em>':'')+'</h3><p class="org">'+esc(p.org)+'</p></div>'+
...
function sheet(id){
  ...
  el.innerHTML='<div class="sheet" role="dialog" aria-modal="true" aria-label="'+esc(p.name)+' 簡介"><button class="x" aria-label="關閉">×</button><div class="hd"><div class="ava">'+(p.photo?'<img src="'+p.photo+'" alt="">':esc(initials(p.name)))+'</div><div><h3>'+esc(p.name)+' <small style="font-size:15px;font-weight:500;color:var(--muted)">'+esc(p.title)+'</small></h3><div style="color:var(--muted);font-size:15px">'+esc(p.org)+'</div></div></div>'+

...
function contentView() {
  ...
  return `<div class="editor"><div class="side" role="tablist">${secs.map(s => `<button data-sec="${s[0]}" aria-current="${st.cSec === s[0]}">${s[1]}</button>`).join("")}</div><div>${st.cSec === "agenda" || st.cSec === "people" ? `<div class="note" style="margin-bottom:14px">網站檔案中的議程與貴賓名單已更新時，可按此套用到資料庫（會取代目前的議程、貴賓與交通資料，已上傳的照片會保留）。<div class="tools" style="margin:10px 0 0"><button class="btn sm" data-act="syncagenda">套用最新議程與貴賓名單</button></div></div>` : ""}${secBody()}
...
document.addEventListener("click", async e => {
  ...
  if (a === "camon") camOn(); else if (a === "camoff") camOff();
    ...
    st.C.sessions = F.sessions; st.C.agendaVersion = F.agendaVersion; st.C.travel = F.travel;
